...
function createContextItemControl(id, name, datatype, unit, referencerange, popDialog, wiki, weight) {
    ...
    text += '<img onclick="del(\'' + id + '\');" style="cursor:pointer; position: relative; margin-bottom: -5px" src="/Content/images/trash.png" alt="remove item" />' + '</td></tr>';
            ...
            for (var j = 0; j < innerLength; j++) {
                ...
                if (innerItem.tagName.toLowerCase() == 'input' ||
                    ...
                    if (innerItem.attributes &&
                        ...
                        innerItem.attributes["class"].value == ("string-value"))
                        val = "'" + val + "'";
                    innerExpression += val;
                    innerExpression += ' ';
                }
...
* @param {string} operator Math operator of a condition cluase.
* @param {string} value Right hand side of a condition cluase (rhs).
*
*/
function addRuleElement(type, name, operator, value) {
    ...
    if (type == "and") {
        ...
        item.appendChild(op);
        addSelectOption(op, "&&", getGlobalVar('And'));
        addSelectOption(op, "||", getGlobalVar('Or'));
        addSelectOption(op, "!", getGlobalVar('Not'));
        op.selectedIndex = 0;
    }
    else if (type == "or") {
        var op = document.createElement("select");
        ...
        item.appendChild(rhs);
        addSelectOption(rhs, "true", getGlobalVar('Positive'));
        addSelectOption(rhs, "false", getGlobalVar('Negative'));
        if (value)
            rhs.value = value;
    }
    else if (type == "string") {
